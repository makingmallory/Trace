...
function TrackableCard({ details, onArchive }: { details: TrackableDetails; onArchive: () => void }) {
  return <article className="collection-card">
    <span className="collection-card__icon" aria-hidden="true">{iconGlyph(details.trackable.icon)}</span>
...
export function TrackablesScreen() {
  ...
  return <section className="screen trackables-screen">
    <header className="collection-hero"><div><p className="eyebrow">Your collection</p><h1>Trackables</h1><p className="screen__description">Little pieces of your life, ready whenever you want to check in.</p><p className="collection-count"><strong>{library.active.length}</strong> active Trackable{library.active.length === 1 ? '' : 's'}</p></div><div className="collection-hero__actions"><Link className="bubble-action" to="/trackables/add" aria-label="Add Trackable" title="Add Trackable"><span aria-hidden="true">+</span></Link><Link className="bubble-action" to="/trackables/manage" aria-label="Manage Trackables" title="Manage Trackables"><span aria-hidden="true">⚙</span></Link></div></header>
...
export function CategoriesScreen() {
  ...
  return <Page eyebrow="Manage" title="Categories" description="Arrange the shelves that hold your Trackables." backTo="/trackables/manage">{notice && <p className="notice notice--success" role="status">{notice}</p>}{error && <p className="notice notice--error" role="alert">{error}</p>}<section className="category-manager"><form className="category-create" onSubmit={create}><label className="form-field"><span>New category</span><input value={newCategory} onChange={(event) => setNewCategory(event.target.value)} placeholder="Category name" required /></label><button className="primary-button">Add</button></form><ol className="category-list">{library.categories.map((category, index) => <li key={category.id}><span className="drag-hint" aria-hidden="true">⋮⋮</span><div><strong>{category.name}</strong><small>{category.active ? 'Visible' : 'Hidden'}</small></div><div className="category-actions"><button className="icon-button" aria-label={`Move ${category.name} up`} disabled={index === 0} onClick={() => void action(() => trackableEngine.reorderCategory(category.id, -1), 'Categories reordered.')}>↑</button><button className="icon-button" aria-label={`Move ${category.name} down`} disabled={index === library.categories.length - 1} onClick={() => void action(() => trackableEngine.reorderCategory(category.id, 1), 'Categories reordered.')}>↓</button><button className="text-button" onClick={() => { const name = window.prompt('Rename category', category.name); if (name !== null) void action(() => trackableEngine.renameCategory(category.id, name), 'Category renamed.') }}>Rename</button><button className="text-button" onClick={() => void action(() => trackableEngine.setCategoryActive(category.id, !category.active), category.active ? 'Category hidden.' : 'Category shown.')}>{category.active ? 'Hide' : 'Show'}</button></div></li>)}</ol></section></Page>
}
