...
export function HistoryScreen() {
  ...
  return <section className="screen history-screen">
    ...
    <section className="history-calendar-card" aria-labelledby="history-calendar-heading">
      <div className="history-calendar-toolbar"><div><p className="eyebrow">Calendar</p><h2 id="history-calendar-heading">{calendarTitle}</h2></div><div className="history-calendar-controls"><div className="history-view-switch" aria-label="Calendar view"><button type="button" className={calendarView === 'month' ? 'is-active' : ''} aria-pressed={calendarView === 'month'} onClick={() => changeView('month')}>Month</button><button type="button" className={calendarView === 'week' ? 'is-active' : ''} aria-pressed={calendarView === 'week'} onClick={() => changeView('week')}>Week</button></div><div className="calendar-actions"><button type="button" aria-label={`Previous ${calendarView}`} onClick={() => moveCalendar(-1)}>←</button><button type="button" onClick={() => selectDate(today)}>Today</button><button type="button" aria-label={`Next ${calendarView}`} onClick={() => moveCalendar(1)}>→</button></div><button ref={colorButtonRef} type="button" className={`calendar-color-toggle${colorOpen ? ' is-active' : ''}`} aria-label="Format calendar" aria-expanded={colorOpen} aria-controls="calendar-color-panel" onClick={() => setColorOpen((open) => !open)}><PaletteIcon /></button></div></div>
    ...
    {!hasRecords ? <div className="empty-state"><span aria-hidden="true">◷</span><h2>Your History starts here</h2><p>Complete a Check-In or log an event, and it will appear on this calendar.</p><Link className="primary-button button-link" to="/">Go to Home</Link></div> : detail ? <DayDetail detail={detail} onDelete={remove} /> : null}
  </section>
}
...
function CalendarFormattingPanel({ panelRef, options, totalOptions, selected, metricId, metricQuery, heatmap, valueCount, onQueryChange, onSelect, onHeatmapChange, onClear }: {
  ...
  return <div ref={panelRef} id="calendar-color-panel" className="calendar-color-panel" role="dialog" aria-label="Calendar Formatting">
    ...
    <label className="calendar-metric-search"><span>Color by</span><input type="search" value={metricQuery} onChange={(event) => onQueryChange(event.target.value)} placeholder="Find a Trackable or Event" /></label>
    {totalOptions ? <div className="calendar-metric-options" role="listbox" aria-label="Color calendar by">
      ...
      {!options.length ? <p>No matching Trackables or Events.</p> : null}
    </div> : <p>No supported Trackables or Events are available yet.</p>}
    ...
    <div className="calendar-color-panel__help">{selected && !valueCount ? <p>No historical {selected.kind === 'Event' ? 'events' : 'values'} are recorded for {selected.name} yet.</p> : selected && heatmap ? <><p>Intensity uses the observed historical range.</p><div className="heatmap-legend" aria-label={`${selected.name} heatmap scale from lower to higher`}><span>Lower</span><i aria-hidden="true" /><span>Higher</span></div></> : selected?.kind === 'Event' ? <p>Days with {selected.name} are marked; daily counts stay visible.</p> : selected ? <p>{selected.name} values are shown on applicable days.</p> : <p>Choose a Trackable or Event to color the calendar.</p>}</div>
  </div>
}
...
function SearchResults({ response, onSelect }: { response: HistorySearchResponse; onSelect: (localDate: string) => void }) {
  ...
  if (!response.results.length) return <section className="history-results" aria-live="polite"><div className="section-heading"><div><p className="eyebrow">Search results</p><h2>No matches</h2></div></div><p className="empty-copy">Try an event name, Trackable, option, category, or note text.</p></section>
...
function DayDetail({ detail, onDelete }: { detail: ReturnType<typeof buildDayDetail>; onDelete: (recordId: string, label: string) => Promise<void> }) {
  return <section className="day-detail" aria-labelledby="day-detail-heading"><div className="day-detail__heading"><div><p className="eyebrow">Selected day</p><h2 id="day-detail-heading">{dateLabel(detail.localDate, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</h2></div><Link className="secondary-button button-link" to={`/events?date=${detail.localDate}`}>+ Add event</Link></div>
    ...
    {detail.checkIn ? <article id={`history-record-${detail.checkIn.record.id}`} tabIndex={-1} className="history-record-card history-checkin-card"><div className="history-record__heading"><div><span className={`status-chip status-chip--${detail.checkIn.record.status}`}>{detail.checkIn.record.status === 'completed' ? '✓ Completed' : '◔ Draft'}</span><h3>Nightly Check-In</h3></div><div className="history-record__actions"><RecordAction label="Edit Nightly Check-In" title="Edit Check-In" to={`/history/check-in/${detail.localDate}`}><EditIcon /></RecordAction><RecordAction label="Delete Nightly Check-In" title="Delete Check-In" danger onClick={() => void onDelete(detail.checkIn!.record.id, 'Nightly Check-In')}><TrashIcon /></RecordAction></div></div>
